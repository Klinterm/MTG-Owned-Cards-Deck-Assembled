...
class SimpleCardVisualizer:
    ...
    def create_text_summary(self, parent_widget):
        ...
        text.insert(tk.END, f"  (First 100 cards are the main deck, cards ranked 101-150 are additional suggestions)\n")
...
class MoxfieldAnalyzerApp:
    ...
    def update_auto_include_list(self):
        ...
        # Function to add cards to the UI for a given color key
        def add_cards_to_ui(color_key, cards):
            print(f"Found {len(cards)} cards for {color_key}: {cards}")

            # Add a checkbox for each card
            for i, card in enumerate(cards):
                # Create a variable for this checkbox
                var = tk.BooleanVar(value=self.auto_include_manager.is_card_enabled(color_key, card))
                self.checkbox_vars[card] = var

                # Create a frame for this card entry
                card_entry_frame = tk.Frame(self.checkbox_frame, bg="#f0f0f0")
                card_entry_frame.pack(fill="x", padx=2, pady=1)
                ...
                # Create a label for the card name (smaller width)
                card_label = tk.Label(
                    card_entry_frame,
                    text=card,
                    width=20,
        ...
        if color in self.auto_include_manager.auto_includes:
            ...
            if "_" in color:
                ...
                if alternate_key in self.auto_include_manager.auto_includes:
                    ...
                    add_cards_to_ui(alternate_key, cards)
                    return
    ...
    def setup_setup_tab(self):
        ...
        auto_include_frame = tk.LabelFrame(input_frame, text="Auto-Include Cards", padx=5, pady=5, bg="#f0f0f0")
        auto_include_frame.pack(fill="x", padx=5, pady=2)

        # Top row: color selection + card name
        top_row = tk.Frame(auto_include_frame, bg="#f0f0f0")
        top_row.pack(fill="x", padx=2, pady=1)
        ...
        # Card name entry in same row
        card_name_label = tk.Label(top_row, text="Card:", bg="#f0f0f0")
        card_name_label.pack(side="left", padx=2)

        self.card_name_var = tk.StringVar()
        card_entry = tk.Entry(top_row, textvariable=self.card_name_var, width=25)
        card_entry.pack(side="left", padx=2, fill="x", expand=True)

        # Second row: card type + add button
        type_row = tk.Frame(auto_include_frame, bg="#f0f0f0")
        type_row.pack(fill="x", padx=2, pady=1)

        card_type_label = tk.Label(type_row, text="Type:", bg="#f0f0f0")
        card_type_label.pack(side="left", padx=2)
        ...
        card_type_menu.pack(side="left", padx=2)
        ...
        # Frame for auto-include list and checkboxes - reduced height
        list_frame = tk.Frame(auto_include_frame, bg="#f0f0f0", height=120)
        list_frame.pack(fill="both", expand=True, padx=2, pady=1)
        list_frame.pack_propagate(False)  # Don't let it resize to content
        ...
        # Button frame - with buttons side by side
        ...
        land_frame.pack(fill="x", padx=5, pady=2)
        ...
        # Progress section
        progress_frame = tk.LabelFrame(self.setup_tab, text="Progress", padx=10, pady=5, bg="#f0f0f0")
        progress_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=2)
        ...
        log_frame = tk.LabelFrame(self.setup_tab, text="Log", padx=10, pady=5, bg="#f0f0f0")
        log_frame.grid(row=3, column=0, sticky="nsew", padx=10, pady=2)

        # Configure the setup_tab to allow the log section to expand
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, height=15)  # Increased height to show more lines
    ...
    def run_analysis(self):
        ...
        output_dir_name = f"moxfield_data_{output_name}"

        # Add a commander-specific suffix if there are commander IDs
        if commander_ids:
            commander_hash = hashlib.md5('_'.join(sorted(commander_ids)).encode()).hexdigest()[:6]
            output_dir_name = f"{output_dir_name}_{commander_hash}"

        self.analyzer = MoxfieldAnalyzer(output_dir=output_dir_name)
        self.log(f"Using output directory: {output_dir_name}")
        ...
        # Target deck size is 150 cards: first 100 are the main deck, next 50 are additional suggested cards highlighted in orange
        self.log(f"Target deck size: 150 cards (100 main deck + 50 additional suggestions highlighted in orange)")
    ...
    def run_analysis_thread(self, csv_path, commander_ids, output_name, colors, page_limit):
        try:
            ...
            # Create a cache filename that includes the commander IDs to avoid using wrong deck IDs
            commander_ids_hash = hashlib.md5('_'.join(sorted(commander_ids)).encode()).hexdigest()[:8]
            ids_file = f"{self.analyzer.output_dir}/all_public_ids_{commander_ids_hash}.json"

            # Check if this is a new commander analysis (different from prior runs)
            is_new_commander_analysis = not os.path.exists(ids_file)

            # If we're analyzing new commanders, clear the previously collected decks to ensure fresh analysis
            if is_new_commander_analysis:
                # Clear existing deck files from previous analyses
                decklists_dir = f"{self.analyzer.output_dir}/decklists"
                if os.path.exists(decklists_dir):
                    for filename in os.listdir(decklists_dir):
                        if filename.endswith('.json'):
                            os.remove(os.path.join(decklists_dir, filename))
                    self.log(
                        f"Cleared previous decklists to ensure fresh analysis for commanders: {', '.join(commander_ids)}")

                # Reset the collected decks tracker
                self.analyzer.collected_decks = set()

            # Check if we already have stored IDs
            if os.path.exists(ids_file):
                with open(ids_file, "r") as f:
                    all_public_ids = json.load(f)
                self.log(f"Loaded {len(all_public_ids)} existing deck IDs for commanders: {', '.join(commander_ids)}")
            else:
    ...
    def update_results_view(self):
        ...
        for _, row in df.iterrows():
            ...
            # Add 'extended' tag for cards with ranks 101-150
            rank = row.get('Rank', 0)
            if rank >= 101 and rank <= 150:
                tag = 'extended'  # Light orange highlighting for extended cards (ranks 101-150)
    ...
    def setup_results_tab(self):
        ...
        self.results_tree.tag_configure('extended', background='#FFCBA4')  # Light orange for cards beyond 100
...
class MoxfieldAnalyzer:
    ...
    def generate_recommended_decklist(self, card_frequency, synergy_matrix, cards_per_deck, target_size=150,
                                      colors=None):
        """
